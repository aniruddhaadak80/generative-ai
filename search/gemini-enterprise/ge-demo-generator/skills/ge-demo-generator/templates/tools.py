...
if os.environ.get("ENABLE_WORKSPACE_MCP") == "1" or os.environ.get("ENABLE_WORKSPACE_AUTH") == "1":
    ...
    # =============================================================================
    # Drive writes made AS THE SIGNED-IN USER
    # Everything built here uploads with the token above, so the user talking to
    # the agent OWNS what is created. Office files are import-CONVERTED by Drive
    # v3 into native Google formats; anything else is stored as-is.
    # The only consumer is save_deliverables_to_drive in the Managed Agent block.
    # These helpers stay here, with the Workspace auth plumbing, because that is
    # where the token they need comes from - the Managed Agent block is nested
    # under the same condition.
    # v2.11.0 removed the other consumer: the demo's own sample documents used to
    # be copied into the user's Drive at conversation time, by an explicit tool
    # and unprompted on the first turn. Both are gone. The deploy uploads those
    # documents once and shares the folder, and when it cannot, the completion
    # banner says so and prints the manual command - see setup_and_deploy.sh.
    # =============================================================================
    _MA_DRIVE_CONVERT = {
    ...
    def _create_workspace_httpx_client_factory(mcp_server_url, scopes):
        ...
        def factory(headers=None, timeout=None, auth=None):
            ...
            try:
                ...
                return create_mcp_http_client(headers=clean_headers if clean_headers else None, timeout=timeout, auth=oauth_provider)
            except Exception as ex:
...
# Cloud Scheduler reads a job that carries no time_zone as UTC. This template
# used to hard-code one zone, so a demo fired every schedule in that zone's local
# time whichever zone the request named. The requested zone is passed through
# instead, and kept on the definition so a later update can still move it.
_DEFAULT_TASK_TIME_ZONE = "UTC"


def _resolve_task_time_zone(time_zone: str) -> str:
    """Normalizes the requested time zone of a Cloud Scheduler job.

    Args:
        time_zone: IANA time zone name (e.g. 'America/Los_Angeles'), or empty.

    Returns:
        The trimmed name, or UTC when the caller asked for none. A name this
        runtime does not recognise is passed through rather than replaced, so
        Cloud Scheduler stays the authority on what it accepts and the error it
        returns names the value it refused.
    """
    return (time_zone or "").strip() or _DEFAULT_TASK_TIME_ZONE
...
def register_scheduled_task(
    ...
    task_prompt: str,
    schedule_cron: str,
    time_zone: str = _DEFAULT_TASK_TIME_ZONE,
    ...
    Args:
        ...
        time_zone: IANA time zone the schedule is read in (e.g.
            'America/Los_Angeles'), defaulting to UTC. A cron expression carries
            no zone of its own, so resolve the one the user meant and pass it
    ...
    _tz = _resolve_task_time_zone(time_zone)
...
def update_scheduled_task(
    ...
    Args:
        ...
        time_zone: New IANA time zone (e.g. 'America/Los_Angeles'). Leave empty
            to keep the zone the task is already registered with.
    ...
    # An empty time_zone keeps the zone already on the definition, so moving the
    # clock never quietly resets the zone. A definition written before the zone
    # was recorded carries none, and lands on UTC.
    _tz = _resolve_task_time_zone(time_zone or _def_data.get("time_zone"))
    _def_ref.update({"schedule_cron": schedule_cron, "time_zone": _tz})
...
if os.environ.get("ENABLE_MANAGED_AGENT") == "1":
    ...
    def _ma_override_tools(include_mcp=True):
        ...
        section 23. The brief no longer promises it and the root agent now
    ...
    def register_scheduled_autonomous_task(
        ...
        Args:
            ...
            schedule_cron: Cron expression (e.g. '0 8 * * 1-5' for 08:00 in
                `time_zone`).
            input_data: Optional data to embed verbatim into every fire.
            time_zone: IANA time zone the schedule is read in (e.g.
                'America/Los_Angeles'), defaulting to UTC. A cron expression
                carries no zone of its own, so resolve the one the user meant and
                pass it here rather than assuming one.
    ...
    if os.environ.get("ENABLE_WORKSPACE_MCP") == "1" or os.environ.get("ENABLE_WORKSPACE_AUTH") == "1":
        ...
        # _MA_DRIVE_CONVERT / _MA_DRIVE_MAX_BYTES / _ma_drive_multipart are defined
        # with the Workspace auth plumbing near the top of this file, next to the
        # token they upload with. This block is nested under the same condition.
        # =============================================================================
    ...
    async def delegate_autonomous_task(*args, **kwargs):
        return {"status": "disabled", "message": "Autonomous delegation is not enabled for this demo agent."}
    def get_autonomous_task_status(*args, **kwargs):
        return {"status": "disabled", "message": "Autonomous delegation is not enabled for this demo agent."}
    def register_scheduled_autonomous_task(*args, **kwargs):
        return {"status": "disabled", "message": "Autonomous schedules are not enabled for this demo agent."}
    def save_deliverables_to_drive(*args, **kwargs):
        return {"status": "disabled", "message": "Drive handoff is not enabled for this demo agent."}
